...
# my files
from property_handler import property_calc, similarity_calc, smiles2fingerprint, is_valid_molecule, find_similar_mol
...
def input2output(args, input_batch, model_in, T1, T2, model_out, random_seed_list=None, max_out_len=90, recover_seed=True):

    input_batch, fp_mols = input_batch
    ...
    if args.use_fp:
        ...
        input_batch_fp = torch.tensor([[float(dig) for dig in fp_mol] for fp_mol in input_batch_fp_str]).to(model_in.device)
    ...
    for seed in random_seed_list:
        ...
        # embedder encode (MMT-Net)
        input_batch_emb, _ = model_in.forward_encoder(input_batch)

        if args.use_baseline_1:  # ablation
            translated_batch_emb = T1(input_batch_emb, input_batch_fp)
        else:  # regular
            translated_batch_emb = T1(input_batch_emb, input_batch_fp)
            translated_batch_emb = T2(translated_batch_emb, input_batch_fp)

        # embedder decode (decode test = input is <bos> and multi for next char + embedding) (MMT-Net)
...
def input2all(args, input_batch, model_in, T1, T2, model_out, max_out_len=90):
...
def generate_results_file(test_loader, input2output_func, input2smiles, results_file_path):
    ...
    for i, input_batch in enumerate(test_loader):
        current_batch_size = len(input_batch[0])
        ...
        for j, input in enumerate(input_batch):
            ...
            output_molecule_smiles = find_similar_mol(input_molecule_smiles, output_molecule_smiles_list)

            out_mols.append(output_molecule_smiles)
            in_mols.append(input_molecule_smiles)
...
def process_results_file(res_file_path, args, valid_res_file_path, trainset):
    ...
    # valid_res_def = valid_res_def.groupby(['input'], as_index=False).head(1)

    if valid_res_def.empty:
        valid_res_def = pd.DataFrame(columns=['input', 'output', args.property, args.patentability, 'sim', 'novel'])
        valid_res_def = valid_res_def.append({'input':'invalid', 'output':'invalid', args.property:0, args.patentability:0, 'sim':0, 'novel':0}, ignore_index=True)
    else:
        ...
        # add depress_property column for output molecules
        valid_res_def[args.patentability] = np.vectorize(property_calc)(valid_res_def['output'], args.patentability)
...
def valid_results_file_to_metrics(valid_res_file_path, args, num_source_mols):
    ...
    validity, diversity, novelty, property, similarity, SR, PR = [], [], [], [], [], [], []
    for retry_i in range(10):
        validity_mean, diversity_mean, novelty_mean, property_mean, similarity_mean, SR_mean, PR_mean = \
    ...
    validity_np, diversity_np, novelty_np, property_np, similarity_np, SR_np, PR_np = np.array(validity), \
        np.array(diversity), np.array(novelty), np.array(property), np.array(similarity), np.array(SR), np.array(PR)
...
def get_metics_for_sample(valid_df, args, how_many_samples, num_source_mols, seed):
    ...
    # valid_df = valid_df.loc[valid_df.reset_index().groupby(['input'])['sim'].idxmax()]
    ...
    # calculate *** NOVELTY ***
    novelty = valid_df['novel'].mean()

    # calculate *** PROPERTY ***
    property = valid_df[args.property].mean()

    # calculate *** PATENTABILITY ***
    PR = valid_df[args.patentability].mean()

    # calculate *** SIM ***
    sim = valid_df['sim'].mean()

    # calculate *** SR ***
    SR = len(valid_df[(valid_df['sim'] > args.SR_similarity) & (valid_df[args.property] > args.SR_property)
        & valid_df['novel'] & (valid_df[args.patentability] < args.SR_patentability)]) / num_valid_mols

    return validity, diversity, novelty, property, sim, SR, PR