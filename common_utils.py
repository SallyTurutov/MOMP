...
# my files
from property_handler import property_calc, similarity_calc, smiles2fingerprint, is_valid_molecule, score_mol_for_loss
...
def input2output(args, input_batch, model_in, T1, T2, model_out, random_seed_list=None, max_out_len=90, recover_seed=True):
    ...
    for seed in random_seed_list:
        ...
        elif args.no_DEET_Net:  # ablation
...
def process_results_file(res_file_path, args, valid_res_file_path, trainset, patentset):
    res_df = pd.read_csv(res_file_path, header=None, delimiter=' ')
    res_df.rename(columns={0: 'input', 1: 'output'}, inplace=True)

    # filter valid molecules
    valid_res_def = res_df[np.vectorize(is_valid_molecule)(res_df['output'], args.enhanced_property)]

    if valid_res_def.empty:
        valid_res_def = pd.DataFrame(columns=['input', 'output', args.enhanced_property, args.suppressed_property, 'sim', 'novel'])
        valid_res_def = valid_res_def.append({'input':'invalid', 'output':'invalid', args.enhanced_property:0, args.suppressed_property:0, 'sim':0, 'novel':0}, ignore_index=True)
    else:
        # add property column for output molecules
        valid_res_def[args.enhanced_property] = np.vectorize(property_calc)(valid_res_def['output'], args.enhanced_property)
        ...
        valid_res_def[args.suppressed_property] = np.vectorize(property_calc)(valid_res_def['output'], args.suppressed_property, patentset)
...
def get_metics_for_sample(valid_df, args, how_many_samples, num_source_mols, seed):
    ...
    # calculate *** ENHANCED PROPERTY ***
    property = valid_df[args.enhanced_property].mean()

    # calculate *** SUPPRESSED PROPERTY ***
    PR = valid_df[args.suppressed_property].mean()

    # calculate *** SIM ***
    sim = valid_df['sim'].mean()

    # calculate *** SR ***
    SR = len(valid_df[(valid_df['sim'] > args.SR_similarity) & (valid_df[args.enhanced_property] > args.SR_property_QED)
        & (valid_df['novel']) & (valid_df[args.suppressed_property] < args.SR_patentability)]) / num_valid_mols
