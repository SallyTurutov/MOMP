...
# my files
from property_handler import property_calc, canonicalize_smiles, calc_PLall_score
...
def create_dynamic_dataset(property, args):
    if property is 'QED':
        dataset_path = 'dataset/QED/C_train.txt'
        dynamic_dataset_path = 'dataset/QED/C_PLvalues'
    if property is 'DRD2':
        dataset_path = 'dataset/DRD2/C_train.txt'
        dynamic_dataset_path = 'dataset/DRD2/C_PLvalues'

    if args.rebuild_dataset is True:
        data = pd.read_csv(dataset_path, header=None)
        data_list = (data.squeeze()).astype(str).tolist()
        final_list = []

        for molecule_SMILES in tqdm(data_list):
            pl_value = calc_PLall_score(molecule_SMILES)
            final_list.append( (molecule_SMILES, pl_value) )
            pd.DataFrame(final_list).to_csv(dynamic_dataset_path + '_train.txt', header=False, index=False)

    return dynamic_dataset_path


def create_dataset(property, rebuild_dataset, args):
    if property is 'QED':
    ...
    if rebuild_dataset is True:
        ...
        dataA, dataB, dataC, merged_train_set_property = [], [], [], []
        for i, mol_smiles in enumerate(upaired_list_no_dup):
            ...
            if property_val >= low_C:
                dataC.append(mol_smiles)
        ...
        C_train, _, _ = create_dataset_files(dataC, dataset_path + 'C', C_len, 0, 0)
    ...
    return dataset_path + 'A', dataset_path + 'B'
...
class Dataset(object):
    ...
    def __init__(self, args, filename, use_atom_tokenizer=False, isA=False, isC=False, size=None):
        if args.property is 'QED':
            path = 'dataset/QED/Fp_train_QED.txt'
        if args.property is 'DRD2':
            path = 'dataset/DRD2/Fp_train_DRD2.txt'
        fps_mols = pd.read_csv(path, header=None).iloc[:, 0].tolist()

        self.isA = isA
        self.isB = not isA and not isC
        self.isC = not isA and isC
        ...
        if isA is False:
            data = self.trainset + fps_mols
        else:
            self.validset = pd.read_csv(filename + '_validation.txt', header=None).iloc[:,0].tolist()
            data = self.trainset + self.validset + fps_mols

        if self.isC:  # C
            pl_values = pd.read_csv(filename + '_train.txt', header=None).iloc[:, 1].tolist()
            tmp_sorted = list(zip(self.trainset, pl_values))
            tmp_sorted.sort(key=lambda x:x[1])
            self.trainset = [mol for mol, _ in tmp_sorted][:(len(self.trainset) // 2)]
            print("dataset size: " + str(len(self.trainset)))
        elif self.isB:  # B
            pl_values = pd.read_csv(filename + '_train.txt', header=None).iloc[:, 1].tolist()
            tmp_sorted = list(zip(self.trainset, pl_values))
            tmp_sorted.sort(key=lambda x: x[1])
        ...
        if self.isA and size is not None:  # A
            self.trainset = self.trainset[:size] if size < len(self.trainset) else self.trainset
