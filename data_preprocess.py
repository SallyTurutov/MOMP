...
# my files
from property_handler import property_calc, canonicalize_smiles
...
def create_dataset(property, rebuild_dataset, patentset, args):
    ...
    if rebuild_dataset is True:
        ...
        for i, mol_smiles in enumerate(upaired_list_no_dup):
            ...
            if property_val <= high_A:
                ...
                suppressed_property_val = property_calc(mol_smiles, args.suppressed_property, patentset)
                if suppressed_property_val < PL_low_C:
        ...
        print('Domain C: ' + str(C_len))

        train_size = B_len
        patentable_size = C_len if C_len < train_size else train_size
        ...
        C_train, _, _ = create_dataset_files(dataC, dataset_path + 'C', patentable_size, 0, 0)
        ...
        ###########  merged not-patents set for baseline testing ################
        merged_train = B_train + C_train
        pd.DataFrame(merged_train).to_csv(dataset_path + property + '_mergedBP_specific_baseline.txt', header=False,
                                          index=False)

        ########################################################################

    return dataset_path + 'A', dataset_path + 'B', dataset_path + 'C'
...
class Dataset(object):
    ...
    def __init__(self, filename, use_atom_tokenizer=False, isA=False):
...
def filname2patentset(patentset_filename, drugs=False):
    print(' ')

    # create patentset loader from file
    if drugs:
        df = pd.read_csv(patentset_filename)
        patentset = set(df['smiles'].dropna())
    else:
        df = pd.read_csv(patentset_filename, header=None)
        patentset = set(df.iloc[:, 0])
    print('Initial number of molecules: ' + str(len(patentset)))
    return patentset
