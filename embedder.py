...
class Embedder(nn.Module):
    def __init__(self, dataset, name, args):
        ...
        self.simple_adapt_dim = nn.Linear(encoder_GRU_out_dim, final_embedding_dim)
    ...
    def forward(self, tuple_strings, embedding=None, T=None, generator=False, get_recon_strings=False):
        embedding, KL_loss = self.forward_encoder(tuple_strings)
        recon_loss, _, _ = self.forward_decoder(tuple_strings, embedding, get_recon_strings)
        return KL_loss, recon_loss


    def forward_generator(self, tuple_strings, embedding):
        return self.forward_decoder(tuple_strings, embedding, get_recon_strings=True)
    ...
    def forward_decoder(self, tuple_strings, embedding, get_recon_strings=False):
        ...
        recon_loss = F.cross_entropy(strings_recon_score[:, :-1].contiguous().view(-1, strings_recon_score.size(-1)),
                                          strings[:, 1:].contiguous().view(-1),
                                          ignore_index=self.dataset.c2i['<pad>'])

        recon_strings = None
        if get_recon_strings:
            recon_strings = self.decoder_test(max_len=90, embedding=embedding, get_output_as_string=False)
        return recon_loss, strings_recon_score, recon_strings


    def decoder_test(self, max_len=100, embedding=None, temp=1.0, get_output_as_string=True):
        with torch.no_grad():
...
def save_checkpoint_embedder(current_criterion, best_criterion, model, args):
    ...
    if best_criterion is None or current_criterion > best_criterion:
        ...
        checkpoint_filename_path = args.checkpoints_folder + '/' + args.suppressed_property + '/CDN/checkpoint_model.pth'
