...
# main train function
def fit(args, model, epochs, patentset, is_validation = False):
    ...
    for epoch in range(1, epochs+1):   # [1,2,...,epochs]
        ...
        if is_validation is True and (epoch == 1 or epoch % args.validation_freq == 0):
            validation(args, model.name, model, patentset)
...
# validation function
def validation(args, model_name, model, patentset):
    # evaluation mode
    model.eval()
    ...
    avg_similarity, avg_property, avg_SR, avg_PR, avg_validity, avg_novelty, avg_diversity = \
        general_validation(args, input2output, input_tensor2string, valid_loader, validset_len, model_name, trainset, patentset)
