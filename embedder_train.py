...
from property_handler import find_similar_mol
...
# main train function
def fit(args, model, epochs, is_validation = False):

    if epochs == 0:
        return
    # data loaders
    train_loader = get_dataloader(model, args, model.dataset.get_trainset(), shuffle=True)
    ...
    for epoch in range(1, epochs+1):   # [1,2,...,epochs]
        ...
        #     validation(args, model.name, model)
...
# validation function
def validation(args, model_name, model):
    # evaluation mode
    model.eval()
    ...
    trainset = set(model.dataset.get_trainset())

    # use general validation function
    avg_similarity, avg_property, avg_SR, avg_PR, avg_validity, avg_novelty, avg_diversity = \
        general_validation(args, input2output, input_tensor2string, valid_loader, validset_len, model_name, trainset)
...
def get_dataloader(model, args, data, batch_size=batch_size, collate_fn=None, shuffle=True):
    ...
    fpData = pd.read_csv(path, header=None).iloc[:, 0].tolist()

    if collate_fn is None:
        def collate_fn(train_zipped_data):
            train_data = [i for i, j in train_zipped_data]
            fp_data = [j for i, j in train_zipped_data]
            ...
            for string in train_data:
                tensors.append(model.dataset.string2tensor(string, model.dataset.c2i, device=model.device))
                mols.append(model.dataset.string2tensor(find_similar_mol(string, fp_data), model.dataset.c2i, device=model.device))
            # tensors = [model.dataset.string2tensor(string, model.dataset.c2i, device=model.device) for string in train_data]
            return tensors, mols
    return DataLoader(list(zip(data, fpData)), batch_size=batch_size, shuffle=shuffle, collate_fn=collate_fn, num_workers=0, worker_init_fn=set_seed(args.seed))
