...
def smiles2mol(molecule_SMILES):
    ...
    if mol is None:
        # print("Exception has occurred in smiles2mol")
        raise Exception()
    return mol
...
def smiles2fingerprint(molecule_SMILES, radius=2, nBits=2048, useChirality=False, fp_translator=False):
    try:
        ...
        if fp_translator is True:
            fp = AllChem.GetMorganFingerprintAsBitVect(mol, radius=radius, nBits=2048, useChirality=useChirality)
        else:
            fp = AllChem.GetMorganFingerprintAsBitVect(mol, radius=radius, nBits=nBits, useChirality=useChirality)
        return fp
    except Exception as e:
        print("Exception has occurred in smiles2fingerprint")
        raise Exception()
...
def property_init(args):
    ...
    global trained_property
    trained_property = args.property

    global patentsFp_list
    patentsFp_path = 'dataset/PL/patentsFp.pkl'
    patentsFp_file = open(patentsFp_path, "rb")
    patentsFp_dict = pickle.load(patentsFp_file)
    patentsFp_list = list(patentsFp_dict.items())
...
def property_calc(molecule_SMILES, property):
    ...
    if property == 'QED':
        ...
        print(str(property) + ' doesn\'t exist')
        raise Exception()
...
def similarity_calc(molecule_SMILES_1, molecule_SMILES_2):
    try:
        ...
        print("Exception has occurred in similarity_calc")
        raise Exception()
...
def canonicalize_smiles(molecule_SMILES):
    try:
        ...
        print("Exception has occurred in canonicalize_smiles")
        raise Exception()
...
def calc_PLall_score(molecule_SMILES):
    ...
    return np.max(np.array([f_sim(patent_fp_tuple) for patent_fp_tuple in patentsFp_list]))
...
def find_similar_mol(molecule_SMILES, molecule_SMILES_group):
    sim = 0
    mol = ''
    for crt_mol in molecule_SMILES_group:
        if is_valid_molecule(crt_mol, trained_property):
            crt_sim = similarity_calc(molecule_SMILES, crt_mol)
            if crt_sim > sim:
                sim = crt_sim
                mol = crt_mol
